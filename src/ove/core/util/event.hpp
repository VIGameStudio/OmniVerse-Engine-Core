...
namespace ove {
	namespace core {
		...
		template <typename E>
		struct signal_t : public signal_base_t {
		...
		private:
			friend struct event_bus_t;
			std::function<void(std::vector<sig_handle>&)> m_clearSigFn;
			std::vector<sig_handle> m_sigHandles;
		};

		struct event_bus_t : nocopy {
		public:
			using sig_sptr = std::shared_ptr<signal_base_t>;
			...
			void subscribe(R& receiver)
			{
				if (receiver.m_sigHandles.size() == 0) {
					receiver.m_clearSigFn = std::bind(&event_bus_t::clear, this, std::placeholders::_1);
				}
